...
class DataService:
    ...
    @staticmethod
    def get_products():
        """Получает все товары с подстановкой названий вместо ID через JOIN"""
        conn = get_connection()
        try:
            cur = conn.cursor(dictionary=True)
            cur.execute("""
    ...
    def save_product(model: dict):
        ...
        try:
            ...
            manufacturer_id = DataService.ensure_manufacturer(conn, model["manufacturer_name"])
            cur = conn.cursor()
            if model.get("product_id"):
                ...
                """, (
                    ...
                    manufacturer_id, supplier_id, model["price"], model["unit_name"],
    ...
    def get_next_order_number():
        """Генерирует следующий номер заказа: MAX + 1"""
        conn = get_connection()
        try:
            cur = conn.cursor()
    ...
    def get_next_pickup_code():
        """Генерирует код получения: MAX + 1, начиная с 900"""
        conn = get_connection()
        try:
            cur = conn.cursor()
    ...
    def get_discount_ranges():
        """Диапазоны скидок для фильтрации по заданию"""
        return [
            ("Все диапазоны", (None, None)),
            ("0-12.99%", (0, 12.99)),
            ("13-16.99%", (13, 16.99)),
            ("17% и более", (17, 999))
        ]
...
class LoginDialog(QDialog):
    def __init__(self):
        ...
        btn_login.setStyleSheet(f"background-color: {COLOR_ACCENT}; color: white; padding: 5px; border-radius: 3px;")
        btn_guest.setStyleSheet(f"background-color: {COLOR_ACCENT}; color: white; padding: 5px; border-radius: 3px;")
...
class ProductFormDialog(QDialog):
    def __init__(self, product_id=None):
        ...
        fields = [
            ...
            ("Производитель:", self.manufacturer_edit),
        ...
        self.photo_label.setStyleSheet(f"border: 1px solid {COLOR_ACCENT}; background-color: {COLOR_SECOND};")
        self.photo_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        btn_photo = QPushButton("Выбрать фото")
        btn_photo.setStyleSheet(f"background-color: {COLOR_SECOND}; padding: 5px; border-radius: 3px;")
        ...
        btn_save.setStyleSheet(f"background-color: {COLOR_ACCENT}; color: white; padding: 5px; border-radius: 3px;")
        btn_back.setStyleSheet(f"background-color: {COLOR_SECOND}; padding: 5px; border-radius: 3px;")
    ...
    def resolve_photo_path(self, photo_file: str):
        """Определяет путь к файлу фото или возвращает картинку-заглушку"""
        if photo_file:
    ...
    def load_product(self, product_id: int):
        ...
        self.manufacturer_edit.setText(row["manufacturer_name"] or "")
        self.supplier_combo.setCurrentText(row["supplier_name"] or "")
    ...
    def choose_photo(self):
        """Диалог выбора фото с проверкой размера 300x200"""
    ...
    def save_click(self):
        """Сохраняет товар и копирует фото в папку приложения"""
...
class OrderFormDialog(QDialog):
    def __init__(self, order_id=None):
        ...
        self.generated_pickup_code = 900
...
class OrdersDialog(QDialog):
    def __init__(self, user_data: UserInfo, parent=None):
        ...
        btn_back = QPushButton("Назад")
        self.btn_add.setStyleSheet(f"background-color: {COLOR_ACCENT}; color: white; padding: 5px; border-radius: 3px;")
        self.btn_edit.setStyleSheet(f"background-color: {COLOR_SECOND}; padding: 5px; border-radius: 3px;")
        self.btn_delete.setStyleSheet(f"background-color: {COLOR_SECOND}; padding: 5px; border-radius: 3px;")
        btn_back.setStyleSheet(f"background-color: {COLOR_SECOND}; padding: 5px; border-radius: 3px;")
        ...
        self.table.setStyleSheet(f"""
            QTableWidget {{
                ...
                alternate-background-color: {COLOR_SECOND};
...
class ProductsWindow(QMainWindow):
    def __init__(self, user_data: UserInfo):
        ...
        if os.path.exists(ICON_ICO):
            self.setWindowIcon(QIcon(ICON_ICO))
        self.setStyleSheet(f"QMainWindow {{ background-color: {COLOR_WHITE}; }}")

        root = QWidget()
        root.setStyleSheet(f"background-color: {COLOR_WHITE};")
        ...
        if os.path.exists(ICON_PNG):
            ...
            logo.setPixmap(pix)
        title = QLabel("ООО «ЧитайГород» — Список товаров")
        ...
        self.btn_orders.setStyleSheet(f"background-color: {COLOR_SECOND}; padding: 5px; border-radius: 3px;")
        self.btn_orders.clicked.connect(self.open_orders)
        btn_logout = QPushButton("Выход")
        btn_logout.setStyleSheet(f"background-color: {COLOR_ACCENT}; color: white; padding: 5px; border-radius: 3px;")
        ...
        self.search_edit.setStyleSheet(f"border: 1px solid {COLOR_ACCENT}; border-radius: 3px; padding: 3px;")
        filter_layout.addWidget(self.search_edit)
        filter_layout.addWidget(QLabel("Скидка:"))
        self.discount_combo = QComboBox()
        self.discount_combo.setStyleSheet(f"border: 1px solid {COLOR_ACCENT}; border-radius: 3px; padding: 3px;")
        filter_layout.addWidget(self.discount_combo)
        filter_layout.addWidget(QLabel("Сортировка:"))
        self.sort_combo = QComboBox()
        self.sort_combo.setStyleSheet(f"border: 1px solid {COLOR_ACCENT}; border-radius: 3px; padding: 3px;")
        self.sort_combo.addItems(["Без сортировки", "Цена (по возрастанию)", "Цена (по убыванию)", "Остаток (по возрастанию)", "Остаток (по убыванию)"])
        ...
        self.discount_combo.currentTextChanged.connect(self.apply_filters)
        self.sort_combo.currentTextChanged.connect(self.apply_filters)

        self.apply_role_rules()
        self.load_discount_ranges()
        self.load_products()
    ...
    def load_discount_ranges(self):
        ...
        for name, _ in DataService.get_discount_ranges():
...
def main():
    ...
    palette = QPalette()
    palette.setColor(QPalette.ColorRole.Window, QColor(COLOR_WHITE))
    palette.setColor(QPalette.ColorRole.WindowText, QColor(0, 0, 0))
    palette.setColor(QPalette.ColorRole.Base, QColor(COLOR_WHITE))
    palette.setColor(QPalette.ColorRole.AlternateBase, QColor(COLOR_SECOND))
    palette.setColor(QPalette.ColorRole.Text, QColor(0, 0, 0))
    palette.setColor(QPalette.ColorRole.Button, QColor(COLOR_SECOND))
